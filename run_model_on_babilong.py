...
def main(
	...
	system_prompt: str, load_in_8bit: bool, load_in_4bit: bool, use_flash: bool
) -> None:
	"""
	...
	for task in tqdm(tasks, desc='tasks'):
		...
		for split_name in tqdm(split_names, desc='lengths'):
			# load dataset
			print('\nLOADING',dataset_name,split_name,task)
			task_data = datasets.Dataset.from_json(dataset_name+'/'+split_name+'/'+task+'.json')
			#task_data = data[task]
			...
			for sample in tqdm(task_data, desc=f'task: {task} length: {split_name}'):
				...
				df.to_csv(outfile)
				del target, output, question, model_inputs
...
if __name__ == '__main__':
	...
	main(args.results_folder, args.model_name, args.model_path,  args.tokenizer_name, args.tokenizer_path,
		 ...
		 args.system_prompt, args.load_in_8bit, args.load_in_4bit, args.use_flash)
