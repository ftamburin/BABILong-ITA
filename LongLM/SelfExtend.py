...
def apply(loaded_model, group_size, window_size, enable_flash_attention=False, scale_base=-1, flash_attention_impl="flash_attn"):
	'''
		loaded_model: 
	...
	if 'Llama' in arch_name:
		...
		if enable_flash_attention:
			print('MODIFYING MISTRAL WITH FLASHATTN')
			...
			#modifed_1 = modify_method_of_instance(loaded_model, "MistralAttention", "_flash_attention_forward", SE.selfextend_flash_attn.flash_attention2_forward_with_window_size)
			#modifed_2 = modify_method_of_instance(loaded_model, "MistralAttention", "forward", self_extend_attention_forward)
			if (not modifed_1) or (not modifed_2):
				raise Exception(f"Failed to modify the attention method of {arch_name}")
		else:
			print('MODIFYING MISTRAL WITHOUT FLASHATTN')
			self_extend_attention_forward = partial(SE.Mistral.self_extend_forward,
											group_size_1=group_size, 
											group_size_2=window_size,
											scale_base=scale_base)
			#modifed_2 = modify_method_of_instance(loaded_model, "MistralAttention", "forward", self_extend_attention_forward)
			modifed_2 = modify_method_of_instance(loaded_model, "MistralSdpaAttention", "forward", self_extend_attention_forward)
			if not modifed_2:
				raise Exception(f"Failed to modify the attention method of {arch_name}")
		...
		if enable_flash_attention:
			...
			modifed_1 = modify_method_of_instance(loaded_model, "Gemma3Attention", "_flash_attention_forward", SE.selfextend_flash_attn.flash_attention2_forward_with_window_size)
			modifed_2 = modify_method_of_instance(loaded_model, "Gemma3Attention", "forward", self_extend_attention_forward)
			if (not modifed_1) or (not modifed_2):
				raise Exception(f"Failed to modify the attention method of {arch_name}")
		else:
